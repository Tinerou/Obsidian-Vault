...
Uses decoding logic that uses the opcode (and the function code) to select the proper ROM entry.
Control lines where the value does not matter are called *don't cares*(marked with an X).
- The ROM can use any value, and it won't change the function.

![[image-21.png]]
- *Src1Sel:* Selects register to read for src1. (source 1 select)
- *Src2Sel:* Selects register to read for src2. (source 2 select)
...
- *BranchType:* Indicates type of branch or no branch.
