#ComputerOrganization 

### Implementing Stack Memory in ANNA
REMEMBER: ANNA programs can have all four sections in this order
1) Code (assembly code)
2) Data (global variables)
3) Heap (dynamic memory)
4) Stack (starts at address `0x8000`)
![[image-4.png|250x318]]
#### Calling Convention
**Calling convention:** A set of guidelines that dictate how function calls should be written in assembly.
- Most instruction sets have calling conventions as part of the ISA (Instruction Set Architecture).
- Assembly code that follows the convention can be linked together (even if produced by different compilers)
- Dictate: 
	- How arguments  / return values are passed
	- Certain registers may be used for particular purpose (such as a return address)
	- The contents and order of activation records

#### ANNA Calling Convention
- Register usage:
	- `r4`: Return a value after a function call
	- `r5`: Return address at the beginning of the function call
	- `r6`: Frame pointer throughout the program
	- `r7`: Stack pointer throughout the program
- All parameters must be stored on the stack (registers are not used).
- The return value is stored in `r4` (stack is not used).
- Caller must save values in `r1`-`r5` they want retained after a function.
#### Activation Records in ANNA
- All activation records have the same ordering.
- The number of parameter entries varies depending on the number of parameters.
- Remaining entries are used for local variables and temporary values (order left up to programer).
- Activation record for "main" only has local variables and temporary values.
	- No previous frame
	- No parameters
![[image-5.png|290x288]]
#### Function Calls in ANNA
At the beginning of program:
1) Initialize frame pointer `r6` to have address of the stack(`0x8000`)
	`lli r6 0x00`
	`lui r6 0x80`
2) Set stack pointer `r7` to point at the first address after the "main" activation record.
	`addi r7 r6 <size_of_mains_activation_record>`
	
No work is needed at the end of the program.
### Function Call Example
Convert following C++ code to ANNA Assembly:
![[image-6.png]]
Example: **function.ac**


![[RealFuncEx.pdf]]



Helpful to comment offsets of initial frame pointer (`0x8000`)
```Assembly
# Activation record for main
# 0. a
# 1. b
# 2. c
# 3. total

# r6 frame pointer
# r7 stack pointer
main: lli r6 0x00
lui r6 0x80
addi r7 r6 4 # 4 comes from what is in the activation rec

# a = 2;
lli r1 2 # a = r1
sw r1 r6 0 # using offset 0 we set a = 0

# b = 3;
lli r1 3
sw r6 1

# c = 4
lli r1 4
sw r6 2

# call foo(a, b, c)
lw r1 r6 0 # r1: a
sw r1 r7 1 # a --> param 1
lw r1 r6 1 # r1: b
sw r1 r7 2 # b --> param 2
lw r1 r6 2 # r1: c
sw r1 r7 3 # c --> param 3

lli r1 &foo
lui r1 &foo
jalr r1 r5

# total = < return value (r4) >
sw r4 r6 3

# call bar(b, c)
lw r1 r6 1 # r1: b
sw r1 r7 1 # b --> param 1
lw r1 r6 2 # r1: c
sw r1 r7 2 # c --> param 2

lli r1 &bar
lui r1 &bar
jalr r1 r5

# total = total + < return value (r4) >
lw r1 r6 3
add r1 r1 r4
sw r1 r6 3

# cout << total
lw r1 r6 3 # Line not needed in this case
out r1

# end of main
.halt

# Activation record for bar
# 0. previous frame pointer
# 1. parameter a
# 2. parameter b
# 3. c

bar: sw r6 r7 0 # store previous frame ptr
addi r6 r7 0    # move fram ptr to stack ptr
addi r7 r7 4    # move stack ptr to end

# c = a + b;
lw r1 r6 1 # r1: a
lw r2 r6 2 # r2: b
add r3 r1 r2 # r3: a + b
sw r3 r6 3 # r3: c = a+b

# return c + 2;
lw r4 r6 3
addi r4 r4 2 #  < return val (r4) > = c + 2

addi r7 r6 0   # move stack ptr to frame ptr
lw r6 r7 0     # restore previous frame ptr
jalr r5 r0     # return

# Activation record for foo
# 0. previous frame pointer
# 1. parameter x
# 2. parameter y
# 3. parameter z
# 4. sum1
# 5. sum2
# 6. return address

foo: sw r6 r7 0 # store previous fram ptr
addi r6 r7 0    # move fram ptr to stack ptr
addi r7 r7 7    # move stack ptr to end
sw r5 r6 6      # store return address

# Call bar(x, y)
lw r1 r6 1
sw r1 r7 1  # x --> param 1
lw r1 r6 2
sw r1 r7 2  # y --> param 2

lli r1 &bar
lui r1 &bar
jalr r1 r5

# sum1 = < return val (r4) > 
sw r4 r6 4

# Call bar(y, z)
lw r1 r6 2
sw r1 r7 1  # y --> param 1
lw r1 r6 3
sw r1 r7 2  # z --> param 2

lli r1 &bar
lui r1 &bar
jalr r1 r5

# sum 2 = <return val (r4) >
sw r4 r6 5

# return sum1 + sum2;
lw r1 r6 4
lw r2 r6 5
add r4 r1 r2   # < return value (r4) > = sum1 + sum2

lw r5 r6 6     # restore return address
addi r7 r6 0   # move stack ptr to frame ptr
lw r6 r7 0     # restore previous frame ptr
jalr r5 r0     # return
```

### Recursive Function Example
Class Problem: Write an ANNA function that recursively computes the sum f the first $n$ positive integers as follows:
```C++
unsigned int sum(unsigned int n) {
if (n == 0)
	return 0;
return sum(n-1)+n}
```

``` Assembly
# No activation record for main
# Only using registers temporarily

main: lli r6 0x00
lui r6 0x80     # initialize frame pointer
addi r7 r6 0    # initialize stack pointer

# Get n:
in r1

# Call sum(n)
sw r1 r7 1      # n (r1) -> offset 1
lli r1 &sum
lui r1 &sum
jalr r1 r5

# Display final answer (returned in r4)
out r4
.halt

# end of main

# Activation record for sum:
# 0. Previous frame ptr
# 1. Parameter n
# 2. Return address

sum: sw r6 r7 0    # store prev frame ptr
addi r6 r7 0       # move frame ptr to stack ptr
addi r7 r7 3       # move stack ptr to end
sw r5 r6 2         # save return address

lw r1 r6 1         # n -> r1
beq r1 &then
beq r0 &else

then: lli r4 0     # set return value to 0
beq r0 &return

else: lw r1 r6 1   # n -> r1 
addi r1 r1 -1      # n-1 -> r1
sw r1 r7 1         # n-1 -> param 1

lli r1 &sum
lui r1 &sum
jalr r1 r5         # call sum(n-1)

lw r1 r6 1         # n -> r1
add r4 r4 r1       # set return value to sum(n-1) + n

return : lw r5 r6 2  # restore return address
addi r7 r6 0         # move stack ptr to frame ptr
lw r6 r7 0           # load prev frame ptr
jalr r5 r0           # return

```

Use reference to `0x0000` as `NULL` in ANNA